(() => {
  const getAttributeJSON = (attributeName) => {
    const attribute = document.currentScript.getAttribute(attributeName);
    return JSON.parse(attribute);
  };

  const current_variant = getAttributeJSON('data-current-variant');
  const variants = getAttributeJSON('data-variants');
  const product_type = getAttributeJSON('data-product-type');
  const variant_inventory_quantities = getAttributeJSON('data-variant-inventory-quantities');
  const product_title = getAttributeJSON('data-product-title');
  const product_options = getAttributeJSON('data-product-options');
  const show_low_stock_warning = getAttributeJSON('data-show-low-stock-warning');
  const shop_money_with_currency_format = getAttributeJSON('data-shop-money-with-currency-format');
  const default_image = document.currentScript.getAttribute('data-default-image');
  const first_image_width = getAttributeJSON('data-first-image-width');
  const has_only_default_variant = getAttributeJSON('data-has-only-default-variant');
  const product_is_gift_card = getAttributeJSON('data-product-gift-card');
  const first_image_height = getAttributeJSON('data-first-image-height');
  if (default_image === 'null') {
    default_image = null;
  }

  window.Shopify = window.Shopify || {};
  Shopify.money_format = shop_money_with_currency_format;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializeProductSlider() {
      ...
      function initSlider() {
        ...
        const options = {
          direction: 'ttb',
          height: first_image_height * ((window.innerWidth * 0.5) / first_image_width),
          wheel: true,
          ...
          breakpoints: {
            1100: {
              type: 'loop',
              direction: 'ltr',
              height: first_image_height * ((window.innerWidth * 1) / first_image_width),
              arrows: true,
              perPage: 1,
              pagination: false,
              rewind: false,
    ...
    function initStickyCartBar() {
      ...
      function syncButtons() {
        ...
        const isKlaviyoVisible =
          klaviyoForm && klaviyoForm.style.display !== 'none' && klaviyoForm.style.display !== '';
      ...
      clonedButton.addEventListener('click', function (e) {
        ...
        } else if (!has_only_default_variant) {
          scrollToVariantForm();
        } else {
